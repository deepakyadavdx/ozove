...
//             </div>
//           </div>
//           <div className="flex-wrap row align-items-center">
//             <div
...
function AdvantagesSection() {
  ...
  useEffect(() => {
    if (typeof window !== undefined) {
      if (window.innerWidth <= 991) {
        setIsMobile((prev) => !prev); // Using functional update
      }
    }
  }, [isMobile]); // Adding isMobile to the dependency array
          ...
          <div className="flex-wrap row align-items-center">
            <div
              className={`col-lg-4 col-6 col-md-6 ${
                isMobile ? "px-1 my-1" : "my-4"
              }`}
            >
              <div
                className={`inner_advantage_section flex-column ${
                  isMobile ? "px-1 py-2" : "py-5 px-4"
                }`}
              >
                <Image
                  loader={myLoader}
                  style={{ width: isMobile ? "20%" : "12%" }}
