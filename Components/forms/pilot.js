  ...
  return (
    <Formik
      ...
      onSubmit={async (values, { resetForm }) => {
        setLoading(true);
        const sentObj = {
          subject: `Pilot Launch ${values.plan_type} Request`,
          type: 5,
          ...values,
        };
        const result = await axios.post(`/api/sent-mail`, sentObj);
      ...
      {({ values, handleBlur, handleChange, handleSubmit }) => (
        <>
          <Form onSubmit={handleSubmit} className="pilotformtransition">
              ...
              <div>
                <div className="mb-2 d-flex">
                  <Input
                    ...
                    onChange={handleChange}
                  />
                  <p style={{ color: "#333333", fontSize: 12 }}>
                    I agree to OzOves{" "}
                    <span style={{ color: "#FAAD1D", fontSize: 12 }}>
                      Terms & Conditions
                    </span>{" "}
                    and{" "}
                    <span style={{ color: "#FAAD1D", fontSize: 12 }}>
            ...
            <Button
              className={`pilotbtn py-3 ${
                isTablet ? "w-50 mx-3 my-2" : "w-100 mx-0"
              }`}
              type="submit"
              disabled={loading}
            >
